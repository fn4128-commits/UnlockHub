...
public final class MemoRepository {
    ...
    public int unreadCount() {
        try (Cursor cursor = helper.getReadableDatabase()
                .rawQuery("SELECT COUNT(*) FROM memos WHERE unread = 1", null)) {
    ...
    public List<Memo> pendingReminders(long nowMillis) {
        return query("remind_at > ?", new String[]{String.valueOf(nowMillis)});
    }

    /** 解锁-弹窗加强提醒：开启该标记且仍未读的备忘。 */
    public List<Memo> unlockPopupPending() {
        return query("unlock_popup = 1 AND unread = 1", null);
    }
    ...
    /** 已读满 30 天自动删除（不删置顶，不删仍有未来提醒的）。返回删除条数。 */
    ...
    public int pruneExpiredRead() {
        ...
        return helper.getWritableDatabase().delete(
                "memos",
                "unread = 0 AND pinned = 0 AND read_at > 0 AND read_at < ? AND (remind_at = 0 OR remind_at <= ?)",
